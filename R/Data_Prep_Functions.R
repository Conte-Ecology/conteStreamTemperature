...
prepData <- function (catches_string, springFallBPs, df_covariates_upstream,
  ...
  con <-
    dbConnect(
      drv, dbname = "sheds", host = "osensei.cns.umass.edu",
      user = options("SHEDS_USERNAME"), password = options("SHEDS_PASSWORD")
    )
  qry_daymet <- paste0(
    "SELECT featureid, date, tmax, tmin, prcp, dayl, srad, vp, swe, (tmax + tmin) / 2.0 AS airtemp FROM daymet WHERE featureid IN (",
    catches_string, ") ;"
  )
  ...
  foo <- springFallBPs %>% dplyr::mutate(
    site = as.character(site),
    featureid = as.integer(site), finalSpringBP = ifelse(
      finalSpringBP ==
        "Inf" |
        is.na(finalSpringBP), mean.spring.bp, finalSpringBP
    ),
    finalFallBP = ifelse(
  ...
  fullDataSync <- climateData %>% left_join(df_covariates_upstream,
                                            by = c("featureid")) %>% left_join(dplyr::select(tempDataSync,
                                                                                             date, featureid, site, temp), by = c("date", "featureid")) %>%
    left_join(featureid_huc8, by = c("featureid")) %>% left_join(featureid_lat_lon,
                                                                 by = c("featureid")) %>% dplyr::mutate(year = as.numeric(format(date,
                                                                                                                                 "%Y")), airTemp = (tmax + tmin) /
                                                                                                          2) %>% left_join(dplyr::select(foo,-site), by = c("featureid", "year")) %>% dplyr::mutate(
                                                                                                            finalSpringBP = ifelse(
                                                                                                              finalSpringBP ==
                                                                                                                "Inf" |
                                                                                                                is.na(finalSpringBP), mean.spring.bp, finalSpringBP
                                                                                                            ),
  ...
  fullDataSync <- fullDataSync %>% group_by(featureid, year) %>%
    arrange(featureid, year, dOY) %>% mutate(
      impoundArea = AreaSqKM *
        allonnet, airTempLagged1 = lag(airTemp, n = 1, fill = NA),
      temp5p = rollapply(
        data = airTempLagged1, width = 5,
        FUN = mean, align = "right", fill = NA, na.rm = T
      ),
      temp7p = rollapply(
        data = airTempLagged1, width = 7,
        FUN = mean, align = "right", fill = NA, na.rm = T
      ),
      prcp2 = rollsum(
        x = prcp, 2, align = "right", fill = NA
      ),
      prcp7 = rollsum(
        x = prcp, 7, align = "right", fill = NA
      ),
      prcp30 = rollsum(
        x = prcp, 30, align = "right", fill = NA
      )
    )
  fullDataSync <-
    fullDataSync %>% dplyr::filter(
      dOY >= finalSpringBP &
        dOY <= finalFallBP |
        is.na(finalSpringBP) | is.na(finalFallBP &
                                       finalSpringBP != "Inf" &
                                       finalFallBP != "Inf")
    ) %>% dplyr::filter(dOY >=
                          mean.spring.bp &
  ...
  fullDataSync <-
    fullDataSync %>% mutate(
      ...
      huc8 = as.character(HUC8), huc = as.character(HUC12),
      site = as.numeric(as.factor(featureid))
    )
  fullDataSyncS <-
    stdCovs(x = fullDataSync, y = df_stds, var.names = var.names)
