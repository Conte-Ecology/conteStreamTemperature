...
test_that("individual NA are ignored by summary metrics" {
  ...
  class(df_consec$maxConsec_22)
  df_consec$maxConsec_22 == 
  expect_is(dim(df_consec)[1], length(unique(df$featureid)) * length(unique(df$year)))
})
...
test <- group_by(df, featureid, year) %>%
